...
class TestVoiceAPI:
    ...
    def test_get_empty_fingerprint(self, client, test_novel_id):
        """测试无样本时返回可直接展示的空指纹"""
    ...
    def test_get_empty_fingerprint_with_pov_character(self, client, test_novel_id):
        """测试不存在的 POV 指纹返回空指纹"""
        ...
        # Assert - POV 尚未积累样本时返回空指纹，便于前端直接渲染
        assert response.status_code == 200
